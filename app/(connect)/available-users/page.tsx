...
function AvailableUsersListView() {
          ...
          try {
            const fetchNewUsers = async () => {
              try{
                ...
                const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/search?pageNo=${pageNo}&pageSize=${pageSize}&type=${profession}&isWeek=true&isMonth=true`, {
                  method: 'GET',
                  headers: {
                ...
                if(res.status===200){
                  ...
                  const transformedModels = data.users?.map((user: any) => ({
  ...
  return (
    <div className='overflow-y-scroll no-scrollbar h-full bg-[#F0F0F1]'>
      <header className="sticky top-0 w-full h-[72px] flex items-center justify-center bg-white shadow-lg">
        ...
        <span className="text-[16px] leading-[24px] text-center font-semibold">Available Users</span>
      </header>
...
export default AvailableUsersListView
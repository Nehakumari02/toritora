  ...
  return (
    <div className='flex flex-col h-full'>
      <header className="sticky top-0 w-full h-[72px] flex items-center justify-center bg-white shadow-lg">
        ...
        <span className="text-[16px] leading-[24px] text-center font-semibold">{t("bookSlot")}</span>
      </header>
            ...
            <div className='flex flex-col items-start justify-center gap-1'>
              {loading ?
                ...
                <>
                  ...
                  <span className='font-medium text-[10px] leading-[15px] text-[#999999]'>{t("priceNote")}</span>
                  <span className='font-medium text-[14px] leading-[25px] text-[#999999] flex items-center justify-between flex-wrap gap-2'><span className='text-[#FF0000] flex items-center justify-center gap-2'>{clockIcon} {t("duration")}</span> <span className='text-[#2EC458] flex items-center justify-center gap-2'>{jpyIcon} {hourlyPrice}</span></span>
                </>
              }
            </div>
          </div>
      ...
      <div className='max-w-[800px] mx-auto w-full my-4 space-y-4'>
        <span className='text-[16px] font-semibold m-4'>{t("selectedSlot")}</span>
        <div className='mx-4 p-4 shadow-[0_4px_20px_rgba(0,0,0,0.15)] border-l-[6px] border-[#2EC458] rounded-md flex items-center justify-between'>
          <div className='flex flex-col items-start justify-between gap-2'>
            <span className='text-[14px] font-medium text-[#111111]'>{t("slotChoice")}</span>
            <span className='text-[12px] font-normal text-[#999999]'>{t("slotBookedOn")} <span className='text-[12px] font-normal text-[#111111]'>{format(parseISO(deserializedSlot?.date), "dd-MM-yyyy")}</span></span>
          </div>
          <div>
      ...
      <div className='px-4 py-4 flex items-center justify-center'>
        <Link
          ...
          className='w-[calc(min(100%,800px))] h-[54px] text-[16px] leading-[24px] font-bold text-center bg-secondary flex items-center justify-center text-white rounded-md'>{t("continue")}</Link>
      </div>

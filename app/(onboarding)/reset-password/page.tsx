...
function ResetPassword() {
  ...
  const handleSubmit = async () => {
    if (!password || password === "" || lengthError || uppercaseError || lowercaseError || numberError || specialCharError) {
      toast({
        title: "Error",
    ...
    setSubmitting(true);
    const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/auth/resetPassword`, {
      method: 'POST',
      headers: {
    ...
    if (res.status === 200) {
      toast({
        title: "Success",
        description: `Password reset successfull, redirecting to login page`,
        variant: "success"
      })
  ...
  return (
    <div className='h-[100dvh] flex flex-col'>
      <header className='w-full h-[72px] flex-shrink-0 z-10 sticky top-0 bg-white flex items-center justify-center shadow-lg'>
        ...
        <span className='text-[16px] leading-[24px] text-center font-semibold'>New Password</span>
      </header>
            ...
            <div className='flex flex-col flex-1 mx-5 py-4 space-y-4 overflow-y-scroll no-scrollbar'>
              ...
              <div className='space-y-4'>
                ...
                <div className='space-y-2'>
                  <span className='text-[14px] leading-[21px] font-medium text-[#333333]'>New Password</span>
                ...
                <div className='space-y-2'>
                  <span className='text-[14px] leading-[21px] font-medium text-[#333333]'>Confirm Password</span>
              ...
              <div className='flex-1 py-4 w-full flex-col gap-4 flex items-center justify-center pb-4'>
                <button onClick={handleSubmit} disabled={submitting} className='w-full h-[54px] text-[16px] leading-[24px] font-bold text-center bg-secondary flex items-center justify-center text-white rounded-md'>{submitting ? <Loader2 className='animate-spin' /> : "Change Password"}</button>
              </div>

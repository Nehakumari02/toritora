...
export function UserTile({ user }: UserTileProps) {
  ...
  const location = user.location ? user.location : "na";
  return (
    <div className='bg-white min-h-[226px] w-[172px] flex-shrink-0 rounded-md flex flex-col gap-2 items-center px-[12px] py-[14px]'>
      ...
      <div className='flex flex-col items-start justify-center w-full'>
        ...
        <div className='flex flex-col items-start justify-between w-full gap-2'>
          <span className='flex items-center justify-center gap-2 text-[10px] leading-[15px] font-normal text-[#999999]'>{locationIcon}{t(location)}</span>
          <Link href={`/userDetails?userId=${user.userId}`} className='w-full py-1 bg-primary text-white rounded-sm text-center text-[12px] font-medium leading-[18px]'>{t("view")}</Link>
        </div>
      </div>
    </div>
  );

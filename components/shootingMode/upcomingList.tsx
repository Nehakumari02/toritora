                            ...
                            <div key={index} className='space-y-2'>
                              ...
                              <div className='shadow-[0_4px_20px_rgba(0,0,0,0.15)] rounded-lg overflow-hidden' key={index}>
                                <div key={index} className="p-4 flex flex-wrap gap-2 justify-between items-center">
                                  ...
                                  <div className='flex flex-col gap-2 flex-wrap'>
                                    {reservation.user_id_2?._id === userIdRef.current ? <button className='w-[100px] h-[30px] bg-primary text-white text-[9px] font-semibold flex items-center justify-center rounded-lg'>{t("editRequest")}</button> : <></>}
                                    {/* <button className='w-[100px] h-[30px] bg-primary text-white text-[9px] font-semibold flex items-center justify-center rounded-lg'>View Request</button> */}
                                  </div>
                                </div>
                                <div className='p-4 flex flex-row flex-wrap items-center justify-start gap-2 border-t'>
                                  ...
                                  <div className='flex flex-col items-start justify-center'>
                                    <span className='text-[11px] text-[#000000] font-semibold'>{reservation?.shootingLocation || "Shooting location not provided"}</span>
                                    <span className='text-[10px] text-[#919191] font-normal'>{t("shootingLocation")}</span>
                                  </div>
                                </div>
                                <div className='p-4 border-t'>
                                  <span className='text-[11px] font-medium'>{t("paymentStatus")}: </span><span className={`${reservation?.payment_status !== "paid" ? "text-red-500" : "text-green-500"} text-[12px] font-normal capitalize`}>{reservation?.payment_status}</span>
                                </div>
                                <button onClick={() => { handleGoToLink(`/reservation?reservationId=${reservation._id}`) }} className='w-full text-center h-[42px] flex items-center justify-center bg-secondary text-white font-medium text-[14px]'>{t("useShootMode")}</button>
                              </div>
                            </div>
                          );
                        })}
